...
#define HOME_SCREEN P3_2
#define TIMER_SCREEN P3_1
#define CLOCK_SCREEN P3_0
...
void main (void)
{
	...
	while(1)
	{
		...
		i_measured = imax*vpeak;
		power = 0.50*i_measured*120.0;
		...
		//if (TIMER_SCREEN == 0 && CLOCK_SCREEN == 0){
		//sprintf(buff, "Power = %.3fW", power);
		//LCDprint(buff,1,1);
		//}
		...
		if (TIMER_SCREEN == 0){
			sprintf(buff, "%i:%i", timer_minutes_counter,timer_seconds_counter);
			LCDprint(buff, 1,1);
		}
		...
		//If a pushbutton is pressed display this clock
		if (CLOCK_SCREEN == 0){
			if (seconds_counter > 9 && minutes_counter < 10){
				sprintf(clock_buff, "0%i:%i", minutes_counter,seconds_counter);
				LCDprint(clock_buff, 2,1);
			}
			else if (minutes_counter > 9 && seconds_counter < 10){
				sprintf(clock_buff, "%i:0%i", minutes_counter,seconds_counter);
				LCDprint(clock_buff, 2,1);
			}
			else if (minutes_counter > 9 && seconds_counter > 9){
				sprintf(clock_buff, "%i:%i", minutes_counter,seconds_counter);
				LCDprint(clock_buff, 2,1);
			}
			else {
				sprintf(clock_buff, "0%i:0%i", minutes_counter,seconds_counter);
				LCDprint(clock_buff, 2,1);
			}
		}
	 }  
